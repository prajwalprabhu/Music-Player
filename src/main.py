...
class music_player():
    ...
    def gui_button(self):
        ...
        self.previous_button = Button(
            self.button_window, command=self.previous, image=self.image4)
        self.previous_button.grid(row=0, column=1)
        self.button = Button(self.button_window,
                             command=self.play, image=self.image2)
        self.button.grid(row=0, column=2)
        self.volume_scale = Scale(
            self.button_window, orient='vertical', variable=self.volume, from_=100, to=0)
        self.volume_scale.grid(row=0, column=5)
        self.next_button = Button(
            self.button_window, command=self.next, image=self.image3)
        self.next_button.grid(row=0, column=3)
        self.check_button = Checkbutton(
            self.button_window, text="Say name", variable=self.speak_, onvalue=True, offvalue=False)
    ...
    def gui_data_init(self):
        ...
        for pllist in self.udplay_list:
            self.list_menu.add_command(
                label=pllist, command=lambda: self.play_play_list(pllist))

    def gui_menu(self):
        menu = Menu(self.root)
        File = Menu(menu)
        File.add_command(label="Add Folder ctrl+shift+o",
                         command=self.open_folder)
        File.add_command(label="Close Floder", command=self.close_floder)
        playlist_menu = Menu(menu)
        playlist_menu.add_command(
            label="CreatePlaylist", command=self.creat_playlist)
    ...
    def open_folder(self, event=None, a=None, list_=None):
        ...
        if a == None:
            ...
            if self.file_dir.__contains__(a):
                showinfo("Folder Exict",
                         f"The selected Folder {a} has been alredy added")
                return None
        ...
        if self.playing:
            ...
            if len(file) > 4:
                self.image = PhotoImage(
                    file=join(getcwd(), "img/pause.png"))
                self.button.config(image=self.image)
                try:
    ...
    def get_data(self):
        try:
            ...
            with open(normcase(file), "w") as f:
                f.write(
                    r'{"path":[],"last_played":null,"pos":null,"volume":null,"play_list":{}}')
            self.get_data()
    ...
    def check_end_position(self):
        ...
        try:
            while not self.exited:
                ...
                end_time = round(round(round(self.pos/1000, 2)//60, 2) +
                                 round(round(self.pos/1000, 2) % 60, 2)/100, 3)
    ...
    def close_floder(self):
        ...
        if name == "nt":
            self.close_window.iconbitmap(
                normcase(join(getcwd(), "img/tkicon.ico")))
        for vlue, file in enumerate(self.file_dir):
            self.radio_button = Radiobutton(
                self.close_window, variable=self.radio_value, value=vlue, text=file)
            self.radio_button.pack(anchor=W)
        Button(self.close_window, text="Remove",
               command=lambda: remove_folder(self.radio_value.get())).pack()
        pass
    ...
    def creat_playlist(self):
        def add():
            ...
            if self.udplay_list.__contains__(play_name):
                a = askyesno(
                    "PLaylit", f"The Play list {play_name} Already exist ")
                if a:
            ...
            self.list_menu.add_command(
                label=play_name, command=lambda: self.play_play_list(play_name))
            showinfo("Created Playlist",
                     f"Successfully Created Playlist {play_name}")
            self.play_list_window.destroy()

        def continuee():
            self.play_list_list = Listbox(
                self.play_list_window, width=30, selectmode=MULTIPLE)
            self.play_list_list.pack()
            Label(self.play_list_window,
                  text="Select the Songs to add to the playlist").pack()

            for file in self.file_dir:
                self.open_folder(a=file, list_=self.play_list_list)
            Button(self.play_list_window,
                   text="Add to playlist", command=add).pack()

        self.play_list_window = Toplevel()
        if name == "nt":
            self.play_list_window.iconbitmap(
                normcase(join(getcwd(), "img/tkicon.ico")))
